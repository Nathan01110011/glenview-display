...
class ClockButtonApp(App):
    def __init__(self, **kwargs):
        ...
        self.reserved_screen = None
        self.inuse_screen = None
        self.server_ip = None
    ...
    def release_access(self, *_):
        ...
        try:
            ...
            print("⚠️ Release failed:", exc)
            self.try_rescan_server()

    def check_state(self, _):
        try:
            response = httpx.get(self.get_server_url("/state"), timeout=2)
            ...
            if occupied_by is None:
                ...
                self.inuse_screen.set_content(
                    DOG_NAME, DOG_IMAGES, THEME_COLOR, start_time=start_time
                )
                self.switch_to("inuse")
            else:
                self.reserved_screen.set_content(
                    OTHER_DOG_NAME, OTHER_DOG_IMAGES, start_time=start_time
                )
                self.switch_to("busy")
            ...
            print("⚠️ Failed to get state:", exc)
            self.try_rescan_server()
